const InvoiceTemplate = ({ data }) => {
    return (
        <div className="max-w-3xl mx-auto bg-white text-black p-6 border text-sm">
            ...
            <h1 className="text-lg font-bold mb-2">EXPENSE RECEIPT :</h1>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div><h3 className="font-semibold">Billing To :</h3>
                    <p className="font-medium">{data.shopName}</p>
                </div>
                ...
                <div className="text-right">
                    ...
                    <p>
                        <span className="font-semibold">Date:</span> {new Date(data.expenseDate).toLocaleDateString("en-GB").replace(/\//g, "-")}
                    </p>

                    <p>
                        <span className="font-semibold">Payment Mode:</span> {data.paymentType}
                    </p>
                </div>
            </div>

            <p className="mb-4">
            Expense recorded for <b>{data.itemName}</b> purchased from
<b> {data.shopName}</b>.
Below are the details of this expense transaction.
            </p>
            ...
            <table className="w-full border text-center mb-4">
                ...
                <tbody>
  <tr>
    <td className="border p-2"></td> {/* Receipt No */}

    <td className="border p-2">
{new Date(data.expenseDate).toLocaleDateString("en-GB").replace(/\//g, "-")}    </td>
